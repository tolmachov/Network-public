...
Сгенерированный RouterOS-скрипт, обновляющий address-list'ы `bypass-vpn` (трафик идёт напрямую, в обход VPN) и `force-vpn` (трафик принудительно через VPN) на основе списка ASN. Генератор живёт в приватном репо (`asn-resolve`, Go) и резолвит ASN → IP-префиксы через RIPE Stat API.
